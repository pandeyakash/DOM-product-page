...
const sort_select = document.querySelector("#sort-by");
const category_select = document.querySelector("#category");

window.addEventListener("load", () => {
  console.log("Load");
  let key = sort_select.value;
  let sortedProduct = sortProduct(key);
...
sort_select.addEventListener("change", () => {
  ...
  let sortedProduct = sortProduct(sort_select.value);
...
category_select.addEventListener("change", () => {
  product_container.innerHTML = "";
  let key = category_select.value;
  let filterArray = filterProduct(key);
...
function filterProduct(key) {
  ...
  let filterArray = products.filter((ele) => {
    return ele.category === key.split("-").join(" ");
  });
  return filterArray;
}
...
function sortProduct(key) {
  let sortedProduct = [];
  if (key.includes("price") && key.includes("ascending")) {
    sortedProduct = products.sort((a, b) => {
      return a["price"] - b["price"];
    });
  } else if (key.includes("price") && key.includes("descending")) {
    sortedProduct = products.sort((a, b) => {
      return b["price"] - a["price"];
    });
  } else if (key.includes("popularity") && key.includes("ascending")) {
    sortedProduct = products.sort((a, b) => {
      return a["rating"]["rate"] - b["rating"]["rate"];
    });
  } else if (key.includes("popularity") && key.includes("descending")) {
    sortedProduct = products.sort((a, b) => {
...
function displayProduct(sortedProduct) {
  console.log(sortedProduct);
  sortedProduct.forEach((ele) => {
    ...
    img.src = ele.image;
    rating.textContent = `${ele.rating.rate} ⭐️ | (${ele.rating.count})`;
    rating.classList.add("rating");

    card.append(img, title, price, description, rating);
